...
num_jobs = 100
samples_per = 500
ellipse_params = [1.08082474e+00, 4.19571915e+00, 1.41835697e-02, 3.67613752e-03]
...
cl_options_base = '--mode ' + mode + ' --num_samples ' + str(samples_per) + ' --ellipse_params'
for param in ellipse_params:
    cl_options_base += ' ' + str(param)
for i in range(num_jobs):
    #determine the starting sample for this batch of samples and apply that to the command line options
    starting_sample = start + i*samples_per
    cl_options = cl_options_base + ' --ss ' + str(starting_sample)     
    
    #create runname
    runname = 'simulated_data_batch_'+str(i) 
        
    #define the local scratch folder
    localdir = "$_CONDOR_SCRATCH_DIR"
    #write out script for job execution
    execfilename = "exec_%04i"%i + '.sh'
    ...
    executefile.write('python ' + working_dir_full+ '/Secondary_radiation/scripts/gen_data_masks2_ring.py ' + cl_options + "\n")
    ...
    jdlfilename = "exec_%04i"%i + '.jdl.base'
