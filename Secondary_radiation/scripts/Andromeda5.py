...
model_func_path = base_path + 'funcs+params_Andromeda5/'


B_params = np.load(model_func_path + 'B_params.npy') #[[B0, RB0], [B1, RB1]]
print('B params: ', B_params)
...
print('H1 scale height params: ', H1_params2)
...
print('H2 scale height params: ', H2_hz_params)
...
print('ion gas params: ', ion_gas_params)
...
#ISRF from starlight
#arguments in units of pc
def rho_star(R, z, simple=False):
    if np.isscalar(R) and np.isscalar(z):
        if simple:
            res= 10*np.exp(-R/4000)*np.exp(-z/2500)
        else:
...
def bsyn(R, z, E):
    return np.multiply.outer(E**2 ,cf.b0_syn*(B(R, z)**2))

def bIC(R, z, E):
    return np.multiply.outer(E**2, cf.b0_IC*(1 + rho_star(R,z)/cf.rho_CMB))
...
def bC(R, z, E):
    return cf.b0_C*nion(R, z)*(1 + np.log(np.multiply.outer(E, 1/nion(R,z)))/82)

def b(R, z, E):
    return bsyn(R, z, E) + bIC(R, z, E) + bbrem(R, z, E)+ bC(R, z, E)
...
#updated integrands
def dDdz(R, z, E):
    return -(D(R,z,E)/(3*B(R,z)))*dBdz(R, z)
def dDdR(R, z, E):
    return -(D(R,z,E)/(3*B(R,z)))*dBdR(R, z)
def tau_b(R, z, E):
    return E/b(R,z,E)
def tau_D(R, z, E):
    rval = np.sqrt(R**2+z**2)
    return 1/((1/cf.cm_per_pc**2)*D(R,z,E)/((rval +0.000000000001)**2)*(1 + z*dDdz(R, z, E)/D(R, z, E) + R*dDdR(R, z, E)/D(R, z, E)))
def tau_int(z, rval, E, D0):
    R = np.sqrt(rval**2-z**2)
    res = np.logical_not(rval==0)*(1/(1/tau_b(R, z, E) + (D0/D0_bench)/tau_D(R, z, E)))
    return res
def D_tau1_int(z, rval, E, D0):
    R = np.sqrt(rval**2-z**2)
    res = np.logical_not(rval==0)*(D0/D0_bench)*D(R,z,E)*tau_int(z, rval, E, D0)
    return res
def D_tau2_int(z, rval, E, D0):
    R = np.sqrt(rval**2-z**2)
    dDdr = dDdR(R, z, E)*R/rval+dDdz(R, z, E)*z/rval
    return np.logical_not(rval==0)*(D0/D0_bench)*dDdr*tau_int(z, rval, E, D0)
def b_tau_int(z, rval, E, D0):
    R = np.sqrt(rval**2-z**2)
    return np.logical_not(rval==0)*b(R, z, E)*tau_int(z, rval, E, D0)
