...
def ddistdr(r, models):
    ...
    for row in models:
        if row[0] == 'exp':
            ...
            gamma = row[2]
            r0 = row[3]
            res -= dist(r, [row])*(gamma/r + (3-gamma)/(r0*(1+r/r0)))
        else:
...
def sync_integrand(integ_vars, indep_vars, funcs, astro_params):
    y = integ_vars[0]
    alpha = integ_vars[1]
    gamma = integ_vars[2]
    l = integ_vars[3]
    ...
    choicelist = [0, 2*np.pi*np.sqrt(3)*e*(me*g_per_GeV)*me*nu**2/(9*B(R, z)*gamma**4*y**2)*\
                  fe(r, me*gamma)*kv(5/3, 4*np.pi*nu*me*g_per_GeV*c/\
                                                        (3*e*B(R, z)*gamma**2*np.sin(alpha)*y))]
