...
model_func_path = base_path + 'funcs+params_Andromeda4/'
...
def tau_int(z, rval, E, D0):
    R = np.sqrt(rval**2-z**2)
    res = np.logical_not(rval==0)*(1/(b(R,z,E)/E + (((D0/D0_bench)/cf.cm_per_pc**2)*D(R,z,E)/((rval +0.000000000001)**2))*(1 + z*dDdz(R, z, E)/D(R, z, E) + R*dDdR(R, z, E)/D(R, z, E))))
    return res
